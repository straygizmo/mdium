...
/// Run a blocking git closure off the main thread. A synchronous Tauri
/// command executes on the main thread, so a slow `git` invocation (large
/// working tree, UNC path, network push/fetch) would block the WebView
...
pub async fn git_init(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["init", "-b", "main"])).await
}

#[tauri::command]
pub async fn git_status(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["status", "--short"])).await
}

#[tauri::command]
pub async fn git_add_all(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["add", "-A"])).await
}

#[tauri::command]
pub async fn git_commit(path: String, message: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["commit", "-m", &message])).await
}

#[tauri::command]
pub async fn git_push(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["push"])).await
}
...
#[tauri::command]
pub async fn git_set_remote_url(path: String, url: String) -> Result<String, String> {
    off_main(move || {
...
#[tauri::command]
pub async fn git_status_porcelain(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["status", "--porcelain=v1"])).await
}
...
#[tauri::command]
pub async fn git_restore_staged(path: String, files: Vec<String>) -> Result<String, String> {
    off_main(move || {
...
pub async fn git_diff_staged(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["diff", "--staged"])).await
}

#[tauri::command]
pub async fn git_diff_unstaged(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["diff"])).await
}

#[tauri::command]
pub async fn git_fetch(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["fetch", "--all", "--prune"])).await
}

#[tauri::command]
pub async fn git_log_oneline(path: String, count: u32) -> Result<String, String> {
    off_main(move || {
        let n = format!("-{}", count);
...
#[tauri::command]
pub async fn git_log_graph(path: String, count: u32, skip: u32) -> Result<String, String> {
    off_main(move || {
        let n = format!("-{}", count);
...
#[tauri::command]
pub async fn git_branch_list(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["branch", "-a", "--no-color"])).await
}

#[tauri::command]
pub async fn git_current_branch(path: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["branch", "--show-current"])).await
}

#[tauri::command]
pub async fn git_switch(path: String, branch: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["switch", &branch])).await
}
...
#[tauri::command]
pub async fn git_remove_untracked(path: String, files: Vec<String>) -> Result<String, String> {
    off_main(move || {
...
#[tauri::command]
pub async fn git_commits_ahead(path: String) -> Result<u32, String> {
    off_main(move || {
...
#[tauri::command]
pub async fn git_push_upstream(path: String, branch: String) -> Result<String, String> {
    off_main(move || run_git(&path, &["push", "-u", "origin", &branch])).await
}

#[tauri::command]
pub async fn git_show_file(path: String, revision: String, file: String) -> Result<String, String> {
    off_main(move || {
