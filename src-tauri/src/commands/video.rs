...
#[tauri::command]
pub async fn video_save_audio(audio_bytes: Vec<u8>, md_path: Option<String>, filename: Option<String>) -> Result<serde_json::Value, String> {
    ...
    let file_path: PathBuf = if let Some(ref name) = filename {
        audio_dir.join(name)
    } else {
        ...
        audio_dir.join(format!("{}.wav", hash))
    };
