...
- コメントがないシーンは、LLMを使って自然なナレーション文を自動生成する（後述「LLMナレーション自動生成」セクション参照）
...
`<!-- narration: ... -->` コメントがないシーンに対し、LLMを使ってスライド内容から自然なナレーション文を自動生成する。

### 生成フロー

1. [▷] 押下時、MD → 中間JSON変換の過程でナレーション未指定のシーンを検出
2. 未指定シーンのMarkdownコンテンツ（タイトル、箇条書き、テキスト等）をLLMに渡す
3. LLMが各シーンのプレゼンテーション用ナレーション文を生成
4. 生成されたテキストを `Scene.narration` に格納
5. ユーザーはシーン編集フォームで確認・修正可能

### LLMプロンプト設計

```
以下のスライド内容から、プレゼンテーション用の自然な話し言葉のナレーションを生成してください。
- 聴衆に語りかける口調で
- 箇条書きの内容を自然な文章に変換
- 30秒〜1分程度で読み上げられる長さ
...
- mdiumの既存AI機能（opencode-sdk）のプロバイダー設定を共有
- OpenAI, Anthropic, Gemini, Ollama等、設定済みのプロバイダーをそのまま利用
- 動画設定UIにLLMプロバイダー選択は不要（アプリ全体の設定に従う）

### 一括 vs 個別

- [▷] 押下時にナレーション未指定の全シーンを一括生成
- シーン編集フォームから個別シーンの再生成も可能（「ナレーション再生成」ボタン）

## 字幕（キャプション）表示

### 概要

ナレーション音声に同期した字幕を動画内に表示する。open-motion componentsの既存Captions実装を活用する。

### 字幕生成フロー

```
Scene.narration テキスト
    ↓  TTS生成時にタイムスタンプ情報を取得
SRTファイル（各シーンごと）
    ↓  parseSrt() で解析
<Captions> コンポーネントで表示
...
- **VOICEVOX**: `/audio_query` のレスポンスに音素ごとのタイミング情報が含まれる。これを文節単位に集約してSRT形式に変換
- **他プロバイダー**: テキスト長とTTS音声長から均等分割で推定。将来的にWhisperベースの音声アライメントで精度向上可能

### 表示スタイル

- 画面下部にオーバーレイ表示（一般的な字幕位置）
- open-motionの `<Captions>` コンポーネントを使用
- フォントサイズ、色、背景の半透明度はデフォルト設定を用意
- シーン編集フォームで字幕のON/OFFを切り替え可能
...
<Sequence from={sceneStart} durationInFrames={sceneDuration}>
  <SceneRenderer scene={scene} />
  <Audio src={scene.narrationAudio} />
  {scene.captions?.enabled && scene.captions?.srt && (
    <Captions srt={scene.captions.srt} />
  )}
</Sequence>
```
