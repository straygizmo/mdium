...
export function CommandsSection() {
  ...
  const handleDelete = async (name: string, itemScope: Scope) => {
    const confirmed = await showConfirm(t("commandDeleteConfirm", { name }), { kind: "warning" });
    if (!confirmed) return;
    if (itemScope === "global") {
                  ...
                  >
                    {missingBuiltins.map((name) => (
                      ...
                      >
                        ...
                        <span className="oc-section__builtin-dropdown-desc">{REGISTRY_COMMANDS[name]?.description ?? ""}</span>
                      </button>
                    ))}
                  </div>
                )}
